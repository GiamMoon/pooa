...
document.addEventListener("DOMContentLoaded", function () {
  const dtSucursal = document.querySelector(".datatables-category-list");
  ...
  const tienePermisoVista = (permisoEsperado) => {
    return permisos.some(p =>
      p.ruta === 'administracion/sucursal' && p.permiso === permisoEsperado
    );
  };
  ...
  const tabla = new DataTable(dtSucursal, {
    ajax: assetsPath + "sucursalAjax/listar",
      columns: [
        { data: "id_sucursal" },      //0
        { data: "nombre_comercial" },    //1
        { data: "direccion" },         //2
        { data: "usuarios_asignados" },    //3
        { data: "activo" },         // 4
        { data: "id_sucursal" }      // 5
      ],
      columnDefs: [
...
$(function() {    
  let isSubmittingRegSucursal = false;

  //Listar
  $('#modalRegistrarSucursal').on('show.bs.modal', function () {
  ...
  // Enviar formulario
  $('#formRegistrarSucursal').on('submit', async function (e) {
    e.preventDefault();
    ...
    enviarRegistroSucursal(form);
  });

  //Función envío
  function enviarRegistroSucursal(form){
    isSubmittingRegSucursal = true;    
    const formData = new FormData(form);

    const submitBtn = $(form).find('button[type="submit"]');
    submitBtn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span> Procesando...');

    fetch(`${assetsPath}sucursalAjax/registrar`, {
      method: 'POST',
      body: formData
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          Toastify({ 
            text: 'Sucursal y Almacén por defecto registrados correctamente ✅', 
            duration: 3000,
            style: { background: '#28a745' } 
          }).showToast();
          bootstrap.Modal.getInstance(document.getElementById('modalRegistrarSucursal')).hide();
...
/* ---- INICIO: EVENTO CAMBIAR ---- */
  ...
  $('#btnConfirmDelete2').on('click', function () {
    ...
    fetch(`${assetsPath}sucursalAjax/eliminar`, {
      ...
      body: new URLSearchParams({ id_sucursal: deleteMarId })
    })
      .then(res => res.json())
      .then(resp => {
        if (resp.success) {
                ...
                fetch(`${assetsPath}perfilAjax/listar_distritos?departamento=${encodeURIComponent(data.departamento)}&provincia=${encodeURIComponent(data.provincia)}`)
                  ...
                  .then(distritos => {
                    ...
                    if (data.distrito) $('#edit-distrito').prop('disabled', false);
                        ...
                        $('#edit-id_sucursal').val(data.id_sucursal);
